...
function combineSkeletons(root) {
  ...
  for (const { bones, boneInverses, meshes } of newSkeletons) {
    const newSkeleton = new THREE25.Skeleton(bones, boneInverses);
    meshes.forEach((mesh) => mesh.bind(newSkeleton, new THREE25.Matrix4()));
  }
}

...
class App extends React.Component<{}, { vrmId: string | null }> {
  ...
  private loadVRM(path: string): Promise<VRM> {
    ...
    return new Promise<GLTF>((resolve, reject) => {
      ...
      .then((gltf: any) => {
        return this.debug ? VRMDebug.from(gltf, {
          springBoneImporter: this.debugOption.disableSpringBoneHelper ? new VRMSpringBoneImporter() : new VRMSpringBoneImporterDebug()
        }, this.debugOption) : VRM.from(gltf);
      })
      .then((vrm: VRM) => {
        this.vrm = vrm;
        this.vrmPath = path;
